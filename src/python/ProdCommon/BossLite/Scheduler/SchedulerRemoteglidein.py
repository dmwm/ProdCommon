...
class SchedulerRemoteglidein(SchedulerInterface) :
    ...
    def singleApiJdl(self, job, requirements=''):
        ...
        jdl += 'output  = %s\n' % job['standardOutput']
        jdl += 'error   = %s\n' % job['standardError']
        # only remap stderr, allow CMSSW.stdout to be accessed over http
        jdl += 'transfer_output_remaps   = "%s=/dev/null"\n' % (job['standardError'])
    ...
    def rsyncFromRemoteHost( self, host, outdir, fileList=[] ):
        """
        Uses rsync (piped over remoteCommand) to move files from schedd
        back here
        """
        filesPerRsync = 500
        outCode = 0
        ...
        subCounter = 1
        while os.path.exists('%s/Submission_%s' % (outdir, subCounter)):
    ...
    def removeGsisshSocket(self):
        ...
        self.logging.info("last used on %s" % lastTime)

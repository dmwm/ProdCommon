...
class SiteDBReport:
    ...
    def __init__(self):
        ...
        self.mySiteDB = SiteDBJSON(params)

    def write_cache(self, name, contents):
        """
        Write the contents of a report into the cache
        in the ~/.cms_sitedbcache directory
        """
        if os.getenv('CMS_SITEDB_CACHE_DIR'):
            dirname  = '$CMS_SITEDB_CACHE_DIR/.cms_sitedbcache/'
        elif os.getenv('HOME'):
            dirname  = '$HOME/.cms_sitedbcache/'
        else:
            import pwd
            dirname = '/tmp/sitedbjson_' + pwd.getpwuid(os.getuid())[0] + '/'
    ...
    def load_report(self, report):
        ...
        try:
            ...
            try:
                signal.alarm(self.alarm_timeout)
                urlresults = self.mySiteDB.load_siteDB_Report(report)
                try:
